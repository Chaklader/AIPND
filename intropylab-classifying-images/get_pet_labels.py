...
def get_pet_labels(image_dir):
    ...
    # Process each filename to create a label
    for filename in filename_list:
        ...
        # Add alphabetic words to pet_name, separated by spaces
        for word in name:
            if word.isalpha():
                pet_name += word + " "
        ...
        # If the filename doesn't already exist as a key in the dictionary,
        # add it with the pet_name as the value in a list
        if filename not in results_dic:
            results_dic[filename] = [pet_name]
        else:
            print("Warning: Duplicate filename", filename)
